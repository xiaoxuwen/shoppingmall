!function (t) {
    ...
    var s = {
        tagClass: function () {
            return "am-badge am-badge-primary"
        },
        ...
        constructor: e, add: function (e, n, i) {
            ...
            if (!(r.options.maxTags && r.itemsArray.length >= r.options.maxTags || e !== !1 && !e)) {
                ...
                if (!e.toString().match(/^\s*$/)) {
                    if (r.isSelect && !r.multiple && r.itemsArray.length > 0 && r.remove(r.itemsArray[0]), "string" == typeof e && "INPUT" === this.$element[0].tagName) {
                        var o = e.split(",");
                        if (o.length > 1) {
                            for (var s = 0; s < o.length; s++) this.add(o[s], !0);
                            return void(n || r.pushVal())
                        }
                    }
                    var u = r.options.itemValue(e), l = r.options.itemText(e), p = r.options.tagClass(e),
                        c = t.grep(r.itemsArray, function (t) {
                    ...
                    if (!c || r.options.allowDuplicates) {
                        if (!(r.items().toString().length + e.length + 1 > r.options.maxInputLength)) {
                            var h = t.Event("beforeItemAdd", {item: e, cancel: !1, options: i});
                            if (r.$element.trigger(h), !h.cancel) {
                                r.itemsArray.push(e);
                                var m = t('<span class="tag ' + a(p) + '">' + a(l) + '<span data-role="remove"></span></span>');
                                if (m.data("item", e), r.findInputWrapper().before(m), m.after(" "), r.isSelect && !t('option[value="' + encodeURIComponent(u) + '"]', r.$element)[0]) {
                                    var f = t("<option selected>" + a(l) + "</option>");
                                    f.data("item", e), f.attr("value", u), r.$element.append(f)
                                }
                                n || r.pushVal(), (r.options.maxTags === r.itemsArray.length || r.items().toString().length === r.options.maxInputLength) && r.$container.addClass("am-tagsinput-max"), r.$element.trigger(t.Event("itemAdded", {
                                    item: e,
                                    options: i
                                }))
                            }
                        ...
                        var d = t(".tag", r.$container).filter(function () {
                            return t(this).data("item") === c
                        });
                        r.options.onTagExists(e, d)
                    }
                }
            }
            ...
            if (a.options.typeaheadjs) {
                ...
                a.$input.typeahead(null, l).on("typeahead:selected", t.proxy(function (t, e) {
                    a.add(l.valueKey ? e[l.valueKey] : e), a.$input.typeahead("val", "")
                }, a))
            }
    ...
    var u = t("<div />");
    t(function () {
        t("input[data-role=tagsinput], select[multiple][data-role=tagsinput], [data-am-tagsinput]").tagsinput()
    })
}(window.jQuery);